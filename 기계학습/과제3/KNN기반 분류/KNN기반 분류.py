...
for i in range(1, 40): # K값의 범위 (1..40)
        knn = KNeighborsClassifier(n_neighbors=i)       # K-최근접 이웃 분류
        knn.fit(X_train, y_train)       # fit
        pred_i = knn.predict(X_test)    # KNN 예측값 구하기
        error.append(np.mean(pred_i != y_test)) # 에러 범위에 넣기

plt.figure(figsize=(12, 6))     # 
plt.plot(range(1, 40), error, color='red', linestyle='dashed', marker='o', markerfacecolor='blue', markersize=10)       # plot그리기
