...
def test_split(index, value, dataset): # 데이터 분할
  ...
  for row in dataset:
    if row[index] < value:  # value보다 작을 경우
      left.append(row)  # 왼쪽의 데이터
    else:                 # 그렇지 않을 경우
      right.append(row) # 오른쪽의 데이터
  return left, right    # 리턴

def gini_index(groups, classes): # 지니개수 계산
...
def split(node, max_depth, min_size, depth): # 분할
  ...
  del(node['groups']) # 더 이상 필요하지 않으므로 노드에서 데이터 삭제
  if not left or not right: # 왼쪽 노드 또는 오른쪽 노드가 비어 있는 경우
    node['left'] = node['right'] = to_terminal(left + right)  # left + right터미널 노드 생성
    return
  if depth >= max_depth:  # 트리의 최대 깊이에 도달했는지 -> 터미널 노드 생성
    node['left'], node['right'] = to_terminal(left), to_terminal(right) # 각각 터미널 노드로 생성
    return
  # 최소 크기에 먼저 도달했는지 or 트리의 추가 분할이 필요한지 확인
  if len(left) <= min_size: # 왼쪽이 최소 사이즈보다 작을 경우
    node['left'] = to_terminal(left)  # 
  else:
    node['left'] = get_split(left)  # 추가 분할
    split(node['left'], max_depth, min_size, depth+1) # 다시 나누기
  #  Right child
  if len(right) <= min_size:  # 오른쪽이 최소 사이즈보다 작을 경우
    node['right'] = to_terminal(right)
  else:
    node['right'] = get_split(right)  # 추가 분할
