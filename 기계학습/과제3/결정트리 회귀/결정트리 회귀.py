...
df = pd.read_csv('housing.data', header=None, sep='\s+')        # 데이터 받아오기
...
tree = DecisionTreeRegressor(max_depth=3)       # 결정트리 회귀 진행    
tree.fit(X,y)   # fit

sort_idx = X.flatten().argsort()        # 작은 값부터 데이터 위치 정렬
plt.scatter(X[sort_idx], y[sort_idx], c='lightblue')    # 산포 그래프 생성
