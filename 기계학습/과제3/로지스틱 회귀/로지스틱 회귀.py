...
dataset = pd.read_csv('로지스틱 회귀/User_Data.csv')    # csv파일 불러오기
                      
x = dataset.iloc[:, [2, 3]].values # 2,3번째 행
y = dataset.iloc[:, 4].values # 4번째 열만
...
xtrain = sc_x.fit_transform(xtrain) # fit() + transform() 2차원으로 변환
xtest = sc_x.transform(xtest)   # 정규화/표준화
